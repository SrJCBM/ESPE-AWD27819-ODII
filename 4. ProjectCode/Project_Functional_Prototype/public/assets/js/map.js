const DEFAULT_MB_TOKEN = 'pk.eyJ1Ijoic3JqY2JtIiwiYSI6ImNtZ3g0eGV5NDAwZzYya3BvdmFveWU2dnEifQ.yYCrLmlo9lW-AJf56akVCw';

// --- Utilidades para sanitizar y persistir token ---
...
function tryPersistToken(token) {
  const clean = sanitizeToken(token);
  if (!clean) return null;
  try {
    localStorage.setItem('mb_token', clean);
  } catch (err) {
    console.warn('No se pudo guardar el token de Mapbox en localStorage:', err);
  }
  return clean;
}

// Intentar leer token previo desde localStorage (puede fallar en modo privado)
let localToken = null;
try {
  localToken = sanitizeToken(localStorage.getItem('mb_token'));
} catch (err) {
  console.warn('No se pudo leer el token de Mapbox desde localStorage:', err);
}

// Prioridad: 1) __CONFIG__.MAPBOX_TOKEN  2) localStorage  3) DEFAULT
const configToken = sanitizeToken(globalThis.__CONFIG__?.MAPBOX_TOKEN);
const effectiveToken = configToken || localToken || DEFAULT_MB_TOKEN;

// Si cambió o no existía, persistir para próximas cargas
if (!localToken || localToken !== effectiveToken) {
  tryPersistToken(effectiveToken);
}

// Establecer token para Mapbox GL
mapboxgl.accessToken = effectiveToken;

const routeStatus = document.getElementById('routeResult');

if (!mapboxgl.accessToken) {
  const warningMsg = 'MAPBOX_TOKEN no configurado. Define MAPBOX_TOKEN en el servidor o guarda "mb_token" en localStorage.';
  console.warn(warningMsg);
  if (routeStatus) {
    routeStatus.textContent = warningMsg;
  }
}
...
document.getElementById('calcRouteBtn').addEventListener('click', async () => {
  ...
  const out = routeStatus || document.getElementById('routeResult');
  ...
  try {
    ...
    out.textContent = `No fue posible calcular la ruta: ${reason}`;
    console.error(e);
  }
});
